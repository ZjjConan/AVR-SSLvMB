import torch 

class AverageMeter(object):
    """Computes and stores the average and current value"""
    def __init__(self, name, fmt=':f'):
        self.name = name
        self.fmt = fmt
        self.reset()

    def reset(self):
        self.val = 0
        self.avg = 0
        self.sum = 0
        self.count = 0

    def update(self, val, n=1):
        self.val = val
        self.sum += val * n
        self.count += n
        self.avg = self.sum / self.count

    def __str__(self):
        fmtstr = "{name} {val" + self.fmt + "} ({avg" + self.fmt + "})"
        return fmtstr.format(**self.__dict__)

class ProgressMeter(object):
    def __init__(self, num_batches, meters, prefix=""):
        self.batch_fmtstr = self._get_batch_fmtstr(num_batches)
        self.meters = meters
        self.prefix = prefix

    def get_message(self, batch):
        entries = [self.prefix + self.batch_fmtstr.format(batch)]
        entries += [str(meter) for meter in self.meters]
        return ('\t').join(entries)

    def _get_batch_fmtstr(self, num_batches):
        num_digits = len(str(num_batches // 1))
        fmt = "{:" + str(num_digits) + "d}"
        return "[" + fmt + "/" + fmt.format(num_batches) + "]"

def accuracy(output, target, topk=(1,)):
    """Computes the accuracy over the k top predictions for the specified values of k"""
    with torch.no_grad():
        maxk = max(topk)
        batch_size = target.size(0)

        _, pred = output.topk(maxk, 1, True, True)
        pred = pred.t()
        correct = pred.eq(target.view(1, -1).expand_as(pred))

        res = []
        for k in topk:
            correct_k = correct[:k].reshape(-1).float().sum(0, keepdim=True)
            res.append(correct_k.mul_(100.0 / batch_size))
        return res

def binary_accuracy(output, target):
    correct = torch.eq(torch.round(output).type(target.type()), target).view(-1)
    res = torch.sum(correct) / (target.size(0) * target.size(1))

    return res.view(1,1)

def parse_gpus(gpu_ids):
    gpus = gpu_ids.split(',')
    gpu_ids = []
    for g in gpus:
        g_int = int(g)
        if g_int >= 0:
            gpu_ids.append(g_int)
    if not gpu_ids:
        return None
    return gpu_ids

def normalize_image(images):
    return (images / 255.0 - 0.5) * 2

    
class ToTensorUint8(object):
    def __call__(self, input):
        return torch.tensor(input, dtype=torch.uint8)

# class ToTensorAndNormalize(object):
#     def __call__(self, input):
#         return 2 * (torch.tensor(input, dtype=torch.float32) / 255.0 - 0.5)
    
class ToTensorAndNormalize(object):

    def __init__(self, normalized_mean=0.5, normalized_std=0.5):
        self.mean = normalized_mean
        self.std = normalized_std
    def __call__(self, inputs):
        if type(inputs) == torch.Tensor:
            return (inputs / 255.0 - self.mean) / self.std
        else:
            return (torch.from_numpy(inputs).type(torch.float32) / 255.0 - self.mean) / self.std